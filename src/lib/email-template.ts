 ...
 *
 * Built with table-based layout and inline styles — the only thing email
 * clients (Gmail, Outlook, Apple Mail) render reliably. The Bonavent logo is
 * referenced as `cid:bonavent-logo`, an inline attachment wired up in the API
 * route, so it renders without depending on a public image URL.
 */
...
/** Escape user-supplied text so it can't break the HTML or inject markup. */
...
/** Preserve line breaks from a textarea when rendering into HTML. */
function nl2br(value: string): string {
    return escapeHtml(value).replace(/\r?\n/g, "<br />");
}

/** Up to two uppercase initials for the sender avatar. */
                ...
                <tr>
                  <td align="center">
                    ...
                    <p style="margin:0 0 4px; font-size:12px; line-height:18px; color:${BRAND.body}; font-weight:600;">
                      Bonavent — your trusted way to book, host, and earn.
                    </p>
...
/* ── Admin notification (lands in the admin inbox) ────────────────────── */
...
/* ── Auto-reply (confirmation to the person who wrote in) ─────────────── */
